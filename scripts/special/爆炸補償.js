...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		text = "請問您是否要領取";
		text += "楓葉點數1000以及#i2450000##z2450000#x10嗎?";
		cm.sendYesNo(text);
		text = "null";
	} else if (status == 1) {
		if (cm.getPlayer().getPrizeLog(log) >= 1) {
		...
		FileoutputUtil.logToFile("logs/Data/爆炸補償活動領取.txt", "領取時間:" + FileoutputUtil.NowTime() + "角色名稱:" + cm.getPlayer().getName() + "角色等級: " + cm.getPlayer().getLevel() + "\r\n");
		cm.dispose();
	} else {
		cm.dispose();
	}
