...
public class BuddyListHandler {
    ...
    public static final void BuddyOperationHandler(final LittleEndianAccessor slea, final MapleClient client) {
        ...
        switch (mode) {
            ...
            case 1: {
                ...
                int buddyWorld = World.Find.findWorld(buddyName);
                ...
                if (buddyChannel > 0) {
                    buddyChar = ChannelServer.getInstance(buddyWorld, buddyChannel)
                ...
                if (player.getWorld() != buddy.getWorld()) {
            ...
            case 2: {
                ...
                final int buddyWorld = World.Find.findWorld(buddyCharId);
                BuddyEntry buddy;

                if (buddyChannel < 0) {
                    buddy = BuddyEntry.getByIdfFromDB(buddyCharId);
                } else {
                    final MapleCharacter buddyChar = ChannelServer.getInstance(buddyWorld, buddyChannel).getPlayerStorage().getCharacterById(buddyCharId);
    ...
    private static void notifyRemoteChannel(final MapleClient c, final int remoteChannel, final int otherCid, final String group, final BuddyOperation operation) {
        ...
        if (remoteChannel > 0) {
            World.Buddy.buddyChanged(otherCid, player.getId(), player.getName(), c.getWorld(), c.getChannel(), operation, group);
        }
    }
}
