...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final int npc, final int mode, String script) {
        ...
        try {
            ...
            if (CheckNpc == null || CheckNpc.getName().equalsIgnoreCase("MISSINGNO")) {
                ...
                c.getPlayer().dropMessage(1, "NPC臨時關閉中...");
                dispose(c);
                return;
            }
            if (c.getPlayer().isGM()) {
    ...
    public final void endQuest(final MapleClient c, final int npc, final int quest, final boolean customEnd) {
        if (!customEnd && !MapleQuest.getInstance(quest).canComplete(c.getPlayer(), null) && !GameConstants.CanDoneQuest(quest)) {
            return;
        }
        final Lock lock = c.getNPCLock();
        //final NPCConversationManager cm = cms.get(c);
    ...
    public final void dispose(final MapleClient c) {
        ...
        if (npccm != null) {
            ...
            if (npccm.getScript() != null) {
                c.removeScriptEngine(doc + "/special/" + npccm.getScript() + ".js");
            }
            if (npccm.getMode() != -1) {
                c.removeScriptEngine(doc + "/npc/" + npccm.getNpc() + "_" + npccm.getMode() + ".js");
            }
            if (npccm.getType() == -1) {
                c.removeScriptEngine(doc + "/npc/" + npccm.getNpc() + ".js");
                c.removeScriptEngine(doc + "/npc/notcoded.js");
            } else {
                c.removeScriptEngine(doc + "/quest/" + npccm.getQuest() + ".js");
            }
        }
