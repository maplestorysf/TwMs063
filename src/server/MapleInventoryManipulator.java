...
//            }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (ServerConfig.LOG_DROPITEM && (source.getEquipOnlyId() > 0 || (GameConstants.isAllScroll(source.getItemId()) || GameConstants.isOpScroll(source.getItemId())))) {
    ...
    public static void removeAllByInventoryId(MapleClient c, long inventoryitemid, int itemid) {
        if (c.getPlayer() == null) {
            return;
        }
        ...
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        IItem copyEquipItems = c.getPlayer().getInventory(type).getItemByInventoryItemId(inventoryitemid);
        if (copyEquipItems != null) {
            removeFromSlot(c, type, copyEquipItems.getPosition(), copyEquipItems.getQuantity(), true, false);
