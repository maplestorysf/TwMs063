...
public class MapleEquipIdOnly {
    ...
    private static List<Quadra<Integer, Long, Long, Integer>> OnlyIDList = new ArrayList();
    ...
    public List<Quadra<Integer, Long, Long, Integer>> getData() {
        return OnlyIDList;
    }

    public void addData(int chrid, long inventoryitemid, long ItemOnlyID, int itemId) {
        OnlyIDList.add(new Quadra(chrid, inventoryitemid, ItemOnlyID, itemId));
    }
                ...
                if (equipOnlyId > 0) {
                    if (checkItems.containsKey(equipOnlyId)) {
                        if (((Integer) checkItems.get(equipOnlyId)) == itemId) {
                            ...
                            addData(chr, inventoryitemid, equipOnlyId, itemId);
                        }
                    } else {
    ...
    public void StartCleaning() {
        ...
        iscleaning = true;
        try {
            final ListIterator<Quadra<Integer, Long, Long, Integer>> OnlyId = getData().listIterator();
            while (OnlyId.hasNext()) {
                Quadra<Integer, Long, Long, Integer> Only = OnlyId.next();
                int chr = Only.getFirst();
                long invetoryitemid = Only.getSecond();
                long equiponlyid = Only.getThird();
                int itemId = Only.getForth();
                int ch = World.Find.findChannel(chr);
                int wl = World.Find.findWorld(chr);

                int alreadyTimes = 0;
                try {
                    Connection con = DatabaseConnection.getConnection();
                    try (PreparedStatement ps = con.prepareStatement("SELECT itemid FROM inventoryitems WHERE equipOnlyId = ?")) {
                        ps.setLong(1, equiponlyid);
                ...
                if (ch < 0 && ch != -10 && ch != -20) {
                    ...
                    if (ch == -10) {
                        ...
                        chrs = ChannelServer.getInstance(wl, ch).getPlayerStorage().getCharacterById(chr);
                    }
                    if (chrs == null) {
                        break;
                    }
                    MapleInventoryManipulator.removeAllByInventoryId(chrs.getClient(), invetoryitemid, itemId);
                }
            }
            clearData();
        } catch (Exception ex) {
